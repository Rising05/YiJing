...
`npm run check:mainland-release` 会检查 `docs/release/MAINLAND_RELEASE_CHECKLIST.md` 是否覆盖正式名称/logo、生产发布资料表、短信/微信登录、隐私政策、用户协议、删除账号、30 天保存策略、内容安全、权限、无广告追踪、真实 AI/生图/对象存储、云服务器、域名、HTTPS、备案和原生打包阻塞项，并确认 README/SPEC 已引用该清单。

`npm run check:release-profile` 会检查 `docs/release/PRODUCTION_RELEASE_PROFILE.md` 是否保留运营主体、品牌、域名/服务器/备案、外部服务商、法务文本、应用商店签名和发布前验证命令等正式发布必填栏位，并确认发布清单、README、SPEC 和脚本入口已互相引用。
