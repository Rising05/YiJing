...
| Phase 4 通义万相 + 图片存储 | In Progress | Agent-AI/Backend/QA | 2026-07-11 | wan2.6-t2i HTTP 入口、图片存储抽象、本地/OSS/S3-compatible provider、prompt 禁令、120 秒可配置超时、网络错误映射和本地 HTTP 契约测试已实现；待真实 Key/账号验证 |
...
| 后端 env 示例完整性门禁 | Agent-Backend/Release/QA | Done | `check:env-examples` 当前检查开发/生产模板的 44 个运行时环境变量 key 完整、顺序一致、无重复和无意外 key；同时扫描后端源码中的直接引用，覆盖新增 LLM/万相请求超时配置，防止运行配置漏写模板。脚本排除 `API_BASE_URL`、`LIVE_AI_SMOKE`、`IMAGE_CLEANUP_DRY_RUN` 等 smoke/维护脚本临时变量，并已纳入 root `check:mvp` | `apps/server/scripts/check-env-examples.mjs`, `apps/server/package.json`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/check-env-examples.mjs`; `npm run check:env-examples`; `npm run check:mvp`; `git diff --check` | 2026-07-11 |
...
| Status | Owner | 更新时间 | 证据/备注 |
| --- | --- | --- | --- |
| In Progress | Agent-AI/Backend/QA | 2026-07-11 | 通义万相调用、存储抽象、prompt 禁令、120 秒可配置超时、网络错误映射和本地 HTTP 契约测试已完成；`smoke:live-ai` 仍需真实 Wanx Key/对象存储账号验证 |
