...
| Phase 2 后端 Mock + MySQL | Ready for QA | Agent-Backend/QA | 2026-07-05 | MySQL migrate 完成，全部 18 项 smoke test 通过（health/unauthorized/invalid login/login/input validation/content safety/text-memory/word-card/regenerate/history/detail/favorite/delete history/quota exhausted/delete account/deleted token rejection） |
...
| QA 回归验收 | Done | Agent-QA | 2026-07-05 | 静态检查通过；后端 API smoke 18 项通过；前端 Chrome headless UI smoke 通过 |
...
| Status | Owner | 更新时间 | 证据/备注 |
| --- | --- | --- | --- |
| In Progress | Agent-AI/Backend | 2026-07-05 | 通义万相调用入口、存储抽象、本地持久化 provider、阿里云 OSS 和 S3-compatible 签名上传/删除接口已完成；真实生图和真实对象存储需配置 Key/账号后验证 |
