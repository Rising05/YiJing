...
| Phase 4 通义万相 + 图片存储 | In Progress | Agent-AI/Backend | 2026-07-05 | ImageService、通义万相 wan2.6-t2i HTTP 同步调用入口、图片存储抽象、本地持久化 provider 和阿里云 OSS 签名上传/删除接口已实现；真实万相和真实 OSS 调用待 API Key/账号验证，S3-compatible 存储仍为后续可选 |
...
| Status | Owner | 更新时间 | 证据/备注 |
| --- | --- | --- | --- |
| In Progress | Agent-AI/Backend | 2026-07-05 | 通义万相调用入口、存储抽象、本地持久化 provider 和阿里云 OSS 签名上传/删除接口已完成；真实生图和真实 OSS 存储需配置 Key/账号后验证，S3-compatible 存储仍为后续可选 |
