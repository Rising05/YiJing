...
| Phase 3 LLM 接入 | In Progress | Agent-AI/QA | 2026-07-11 | OpenAI-compatible client、prompt、JSON/schema/anchor 重试、30 秒可配置超时、网络错误映射和本地 HTTP 契约测试已实现；真实调用 live smoke 入口已提供，待 API Key 验证 |
| Phase 4 通义万相 + 图片存储 | In Progress | Agent-AI/Backend/QA | 2026-07-11 | wan2.6-t2i HTTP、存储 provider、prompt 禁令、请求超时/错误映射和本地契约测试已实现；远程图片下载已补 SSRF/重定向、20MB、格式签名与超时防护；待真实 Key/账号验证 |
...
| 外部 AI HTTP 超时与契约验证 | Agent-AI/Backend/QA | Done | LLM 与通义万相请求新增可配置超时，网络异常/超时分别稳定映射为 `LLM_REQUEST_FAILED` 与 `IMAGE_GENERATION_FAILED`；新增本地 HTTP 契约测试，真实检查请求路径、Bearer 鉴权、JSON body、响应解析、Wanx 禁文字参数、上游失败、空/损坏响应、无图片、连接失败和超时，不调用真实 API | `apps/server/src/modules/ai/ai.service.ts`, `apps/server/src/modules/image/image.service.ts`, `apps/server/scripts/check-ai-http-contract.mjs`, `apps/server/scripts/check-runtime-config.mjs`, `apps/server/scripts/check-env-examples.mjs`, `apps/server/scripts/check-production-config-rules.mjs`, `apps/server/.env.example`, `apps/server/.env.production.example`, `apps/server/package.json`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `docs/release/MAINLAND_RELEASE_CHECKLIST.md`, `SPEC.md` | `npm run build:server`; `npm run check:ai-http-contract`; `npm run check:env-examples`; `npm run check:production-config`; `npm run check:mvp`; `git diff --check` | 2026-07-11 |
| 远程图片下载安全与资源限制 | Agent-Backend/Security/QA | Done | 图片持久化下载仅允许 HTTP/HTTPS 或受限 data:image；解析 DNS 并默认拒绝 loopback、私网、链路本地和保留地址，每次重定向重新校验；仅接受 PNG/JPEG/WebP 且校验文件签名；声明长度和流式读取均限制默认 20MB，下载/OSS/S3 请求默认 30 秒超时；可信内网源可用精确 host/通配子域 allowlist 显式放行 | `apps/server/src/modules/image/storage.service.ts`, `apps/server/scripts/check-image-download-security.mjs`, `apps/server/scripts/check-image-storage.mjs`, `apps/server/scripts/check-runtime-config.mjs`, `apps/server/scripts/check-env-examples.mjs`, `apps/server/.env.example`, `apps/server/.env.production.example`, `apps/server/package.json`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `docs/release/**`, `SPEC.md` | `npm run build:server`; `npm run check:image-storage`; `npm run check:image-download-security`; `npm run check:env-examples`; `npm run check:production-config`; `npm run check:mvp`; `git diff --check` | 2026-07-11 |
...
| 后端 API 负向错误码 Smoke | Agent-Backend/QA | Done | 后端 API smoke 新增 SPEC §15 负向路径覆盖：未登录生成必须返回 `UNAUTHORIZED`，文本超过 500 字返回 `INPUT_TOO_LONG`，单词超过 30 个返回 `TOO_MANY_WORDS`，内容安全拦截返回 `CONTENT_BLOCKED`；后续额度、登录失败、正式登录占位拦截和登录补齐额度已覆盖，并继续覆盖长文本点数和 canonical anchor 回归 | `apps/server/scripts/smoke-api.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/smoke-api.mjs`; `npm run build:server`; `npm run prisma:validate`; `npm run smoke:api` | 2026-07-06 |
...
| 生成额度精确扣减 Smoke | Agent-Backend/QA | Done | 后端 API smoke 在测试登录重置为 `20/0` 后，精确断言短文本生成、长文本生成、单词生成、文本重新生成和单词重新生成依次返回 `19/1`、`18/2`、`17/3`、`16/4`、`15/5` 的 `credits.remaining/credits.used`，防止成功生成未扣减、重复扣减或只返回松散额度字段 | `apps/server/scripts/smoke-api.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/smoke-api.mjs`; `npm run build:server`; `npm run smoke:api`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
| 测试登录失败错误码统一 | Agent-Backend/QA | Done | 测试登录手机号或验证码错误时不再返回私有码 `INVALID_TEST_CODE`，统一返回 SPEC/共享包错误码 `INVALID_INPUT`；后端 API smoke 覆盖错误验证码登录失败断言，并已和后续额度、长文本、canonical anchor 回归一起通过 | `apps/server/src/modules/auth/auth.service.ts`, `apps/server/scripts/smoke-api.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/smoke-api.mjs`; `npm run build:server`; `npm run smoke:api`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| Android APK/AAB 构建验证 | Agent-Release/QA | Ready for QA | 当前 OpenJDK 21 与 Android SDK 已就绪；新增可重复的 Debug/Release 构建命令；Capacitor Android 同步后成功生成 Android Debug 签名 APK、未签名 Release APK 与未签名 Release AAB，正式签名仍需用户安全配置真实 keystore | `apps/mobile/package.json`, `package.json`, `apps/mobile/ANDROID_SETUP.md`, `README.md`, `docs/release/MAINLAND_RELEASE_CHECKLIST.md`, `scripts/check-mainland-release-checklist.mjs`, `SPEC.md` | `npm run build:android:debug`; `npm run build:android:release`; `apksigner verify --print-certs app-debug.apk`; `jarsigner -verify app-release.aab`（确认未签名）; `npm run check:release-env -w apps/mobile -- --strict`; `npm run check:mvp`; `git diff --check` | 2026-07-11 |
...
| 首页 Liquid Glass 标注内容层圆角修复 | Agent-Frontend/QA | Done | 根据用户在浏览器对“文本记忆宫殿”入口的标注复查：评论工具选中的实际节点是首页入口内部 `p-5` 内容层，若只裁切外层卡片仍会在标注中看到直角框；已为两个首页入口内容层新增 `home-entry-content`，让被选中的内层也继承 24px 圆角、overflow 和 `clip-path`，同时补上旧版 `.glass-liquid .glass-fallback` 嵌套路径的圆角兜底；UI smoke 扩展为断言 content 层不裁切且保持圆角，并禁止 fallback 回到 `liquid-glass-react` 内部 wrapper | `apps/mobile/src/pages/HomePage.tsx`, `apps/mobile/src/styles/globals.css`, `apps/mobile/scripts/smoke-ui.mjs`, `apps/mobile/scripts/check-mobile-layout.mjs`, `README.md`, `SPEC.md` | `node --check apps/mobile/scripts/smoke-ui.mjs`; `node --check apps/mobile/scripts/check-mobile-layout.mjs`; `npm run check:mobile-layout`; `UI_BASE_URL=http://127.0.0.1:5173 npm run smoke:ui`; `git diff --check` | 2026-07-06 |
| 首页 Liquid Glass 动效层隔离修复 | Agent-Frontend/QA | Done | 根据用户在浏览器 Comment 1 对“文本记忆宫殿”入口的标注继续修复：截图中的 DOM 仍显示 `.glass-fallback` 可能落入 `liquid-glass-react` 内部 wrapper，导致圆角卡片露出直角边界且左侧内容被裁切；已新增 `.glass-effect-layer` 将库组件隔离为绝对定位背景层，真实内容只在自有 `.glass-fallback` 文档流中渲染；所有 glass 层统一 `box-sizing: border-box`、24px 圆角裁切和 `contain: paint`，UI smoke 新增 effect layer 和 selected content 不贴视口边断言 | `apps/mobile/src/components/LiquidGlassCard.tsx`, `apps/mobile/src/styles/globals.css`, `apps/mobile/scripts/smoke-ui.mjs`, `apps/mobile/scripts/check-mobile-layout.mjs`, `README.md`, `SPEC.md` | `npm run build:mobile`; `npm run check:mobile-layout`; `node --check apps/mobile/scripts/smoke-ui.mjs`; `UI_BASE_URL=http://127.0.0.1:5173 npm run smoke:ui`; `git diff --check` | 2026-07-06 |
| 文本记忆点数与模板锚点对齐 | Agent-Backend/AI/QA | Done | 后端 mock 文本记忆根据 SPEC §7.2 字数区间生成 3-5/5-8/8-12 个记忆点，长文本自动选择容量足够的 12 点模板；文本和单词 mock 的 `anchorKey` 改为共享 canonical templates 中的 anchors，不再生成旧版 `a1/a2` 假锚点；真实 LLM prompt 同步明确字数区间，API smoke 覆盖短文本 3-5 点、长文本 8-12 点和禁止 legacy mock anchor | `apps/server/src/modules/generation/mock-generator.ts`, `packages/prompts/src/textMemoryPrompt.ts`, `apps/server/scripts/smoke-api.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/smoke-api.mjs`; `npm run build:server`; `npm run smoke:api`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| 后端 env 示例完整性门禁 | Agent-Backend/Release/QA | Done | `check:env-examples` 当前检查开发/生产模板的 47 个运行时环境变量 key 完整、顺序一致、无重复和无意外 key；同时扫描后端源码中的直接引用，覆盖 LLM/万相超时、图片存储请求超时、下载大小与可信主机配置，防止运行配置漏写模板。脚本排除 `API_BASE_URL`、`LIVE_AI_SMOKE`、`IMAGE_CLEANUP_DRY_RUN` 等 smoke/维护脚本临时变量，并已纳入 root `check:mvp` | `apps/server/scripts/check-env-examples.mjs`, `apps/server/package.json`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/check-env-examples.mjs`; `npm run check:env-examples`; `npm run check:mvp`; `git diff --check` | 2026-07-11 |
...
| 正式登录生产配置门禁 | Agent-Backend/Release/QA | Done | 后端运行配置新增 `AUTH_FORMAL_PROVIDERS=none|sms|wechat|sms,wechat`；MVP 默认 `none` 时短信/微信正式登录保持关闭并提示风险，启用 `sms` 时必须配置短信服务商、Access Key、签名和模板，启用 `wechat` 时必须配置微信 App ID/Secret；`check:production-config` 新增正式登录关闭安全通过、正式登录启用且配置齐全通过、正式登录启用但缺配置失败三类规则验证 | `apps/server/scripts/check-runtime-config.mjs`, `apps/server/scripts/check-production-config-rules.mjs`, `apps/server/.env.example`, `apps/server/.env.production.example`, `README.md`, `docs/release/MAINLAND_RELEASE_CHECKLIST.md`, `SPEC.md` | `node --check apps/server/scripts/check-runtime-config.mjs`; `node --check apps/server/scripts/check-production-config-rules.mjs`; `npm run check:config`; `npm run check:production-config`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| 正式登录渠道占位护栏 | Agent-Backend/QA | Done | 后端预留 `POST /api/auth/sms-code`、`POST /api/auth/sms-login` 和 `POST /api/auth/wechat-login`；在短信服务商和微信开放平台未配置前统一返回 `FEATURE_NOT_CONFIGURED`，不会误创建用户或签发正式 token；共享错误码和前端文案已补齐，API smoke 覆盖未配置拦截，`check:auth-providers` 纳入 root `check:mvp` 防止占位接口退化 | `apps/server/src/modules/auth/auth.controller.ts`, `apps/server/src/modules/auth/auth.service.ts`, `apps/server/src/modules/auth/dto/sms-auth.dto.ts`, `apps/server/src/modules/auth/dto/wechat-login.dto.ts`, `apps/server/scripts/check-auth-providers.mjs`, `apps/server/scripts/smoke-api.mjs`, `apps/server/package.json`, `packages/shared/src/errors.ts`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/check-auth-providers.mjs`; `npm run check:auth-providers`; `npm run check:error-codes`; `npm run build:server`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| 云服务器 HTTPS 反向代理模板 | Agent-Backend/Release/QA | Done | 新增 `deploy/nginx/yijing.conf.example`，作为云服务器前置 Nginx 模板：包含 80 到 443 跳转、Let's Encrypt 证书占位、`/api/` 后端代理、`/healthz` 到 `/api/health` 健康检查、HSTS、安全响应头、`X-Forwarded-*` headers 和请求体大小限制；`check:deploy-config` 已扩展检查 Nginx 模板关键项，后续只需用正式域名、证书路径和上游地址替换占位值 | `deploy/nginx/yijing.conf.example`, `scripts/check-deploy-config.mjs`, `README.md`, `SPEC.md` | `node --check scripts/check-deploy-config.mjs`; `npm run check:deploy-config`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
| 大陆发布准备清单 | Agent-Release/Compliance/QA | Done | 新增 `docs/release/MAINLAND_RELEASE_CHECKLIST.md`，按产品品牌、账号登录、合规与用户权利、AI/图片/数据、云服务器域名备案、原生打包和发布前验证命令整理中国大陆发布交接事项；新增 `check:mainland-release` 并纳入 root `check:mvp`，检查清单、README、SPEC 和脚本入口保持同步。该清单不是法律意见，正式发布前仍需运营主体和法务复核 | `docs/release/MAINLAND_RELEASE_CHECKLIST.md`, `scripts/check-mainland-release-checklist.mjs`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `SPEC.md` | `node --check scripts/check-mainland-release-checklist.mjs`; `npm run check:mainland-release`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| 移动端生产禁用本地 Mock 回退 | Agent-Frontend/Release/QA | Done | 新增 `localMockPolicy`，将前端本地 mock 登录、文本生成、单词生成和重新生成回退限定在 `import.meta.env.DEV`；生产发布环境后端不可用或残留 `local-mock-token` 时会返回用户可见错误，不再静默离线生成；`check:mobile-runtime-config` 扩展为检查 mock 回退必须受开发环境开关保护 | `apps/mobile/src/services/localMockPolicy.ts`, `apps/mobile/src/stores/authStore.ts`, `apps/mobile/src/pages/TextMemoryPage.tsx`, `apps/mobile/src/pages/WordCardPage.tsx`, `apps/mobile/src/pages/GenerateResultPage.tsx`, `apps/mobile/scripts/check-mobile-runtime-config.mjs`, `README.md`, `SPEC.md` | `node --check apps/mobile/scripts/check-mobile-runtime-config.mjs`; `npm run check:mobile-runtime-config`; `npm run build:mobile`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| 合规文案静态门禁 | Agent-Compliance/QA | Done | 新增 `check:compliance-copy`，扫描隐私政策、用户协议、AI 免责声明、合规页面路由、设置页入口、清缓存/删账号入口，以及文本生成、单词生成、结果页和关于页的 AI 免责声明展示；已纳入 root `check:mvp`，防止 MVP 关键合规文案或入口被后续 agent 误删。该门禁不替代正式法律审核 | `apps/mobile/scripts/check-compliance-copy.mjs`, `apps/mobile/package.json`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `SPEC.md` | `node --check apps/mobile/scripts/check-compliance-copy.mjs`; `npm run check:compliance-copy`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| Android release 签名配置门禁 | Agent-Release/QA | Done | 新增 Android release 签名模板 `apps/mobile/android/keystore.properties.example`，Gradle release 构建在本地存在 `keystore.properties` 时自动使用签名配置；`.gitignore` 禁止提交真实 `keystore.properties`、`*.jks` 和 `*.keystore`；新增 `check:android-release-config` 扫描签名模板、Gradle 接线、文档和仓库树，防止签名密码或 keystore 被误提交；真实 APK/AAB 构建仍需 Java 与 Android SDK | `.gitignore`, `apps/mobile/android/app/build.gradle`, `apps/mobile/android/keystore.properties.example`, `apps/mobile/scripts/check-android-release-config.mjs`, `apps/mobile/ANDROID_SETUP.md`, `apps/mobile/package.json`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `docs/release/MAINLAND_RELEASE_CHECKLIST.md`, `docs/release/PRODUCTION_RELEASE_PROFILE.md`, `SPEC.md` | `node --check apps/mobile/scripts/check-android-release-config.mjs`; `npm run check:android-release-config`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
| 生产发布资料表与门禁 | Agent-Release/Compliance/QA | Done | 新增 `docs/release/PRODUCTION_RELEASE_PROFILE.md`，集中列出正式发布前必须填写的运营主体、品牌、域名/服务器/备案、外部服务商、法务文本、应用商店签名和发布前验证命令；新增 `check:release-profile` 并纳入 root `check:mvp`，同时要求发布清单、README、SPEC 互相引用，防止多个 agent 交接时遗漏发布必填资料 | `docs/release/PRODUCTION_RELEASE_PROFILE.md`, `scripts/check-release-profile.mjs`, `scripts/check-mainland-release-checklist.mjs`, `scripts/check-mvp.mjs`, `package.json`, `README.md`, `docs/release/MAINLAND_RELEASE_CHECKLIST.md`, `SPEC.md` | `node --check scripts/check-release-profile.mjs`; `npm run check:release-profile`; `npm run check:mainland-release`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
| MVP 静态验收聚合命令 | Agent-QA | Done | 新增 root `check:mvp`，按顺序运行 server build、Prisma validate、配置/env 示例完整性/CORS 配置/登录渠道预留/错误码/内容安全/AI 模板/AI 重试/图片 prompt/图片存储/生产配置规则/生产脱敏/部署配置检查、大陆发布清单检查、生产发布资料表检查、mobile build、前端密钥/移动端布局/移动端运行配置/合规文案/权限/广告追踪 SDK/Android release 签名配置/发布元数据检查和发布环境报告；作为多个 agent 交接前的快速静态门禁，不替代需要运行服务的 `smoke:api` 与 `smoke:ui` | `scripts/check-mvp.mjs`, `package.json`, `README.md`, `SPEC.md` | `node --check scripts/check-mvp.mjs`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
...
| Java/Android SDK 未配置 | Android APK/AAB 构建 | 用户/Agent-Release | Resolved | ✅ OpenJDK 21 与 Android SDK 已配置；Debug APK、未签名 Release APK/AAB 构建通过，正式签名待真实 keystore |
...
| Status | Owner | 更新时间 | 证据/备注 |
| --- | --- | --- | --- |
| In Progress | Agent-AI/QA | 2026-07-11 | LLM 接入、prompt、JSON/schema/anchor 重试、30 秒可配置超时、网络错误映射和本地 HTTP 契约测试已完成；`smoke:live-ai` 仍需真实 `LLM_API_KEY` 验证 |
...
| Status | Owner | 更新时间 | 证据/备注 |
| --- | --- | --- | --- |
| In Progress | Agent-AI/Backend/QA | 2026-07-11 | 通义万相调用、存储抽象、prompt 禁令、请求超时/错误映射和本地 HTTP 契约已完成；远程图片下载含 SSRF/重定向、20MB、格式签名与超时防护；`smoke:live-ai` 仍需真实 Wanx Key/对象存储账号验证 |
...
在短信服务商和微信开放平台未配置前，以上正式登录预留接口必须统一返回：

```json
{
  "code": "FEATURE_NOT_CONFIGURED",
  "message": "功能尚未配置，请稍后再试"
}
```

要求：未配置状态下不得创建正式用户、不得发送验证码、不得签发 token。
...
| `FEATURE_NOT_CONFIGURED` | 正式短信/微信登录等外部服务未配置 | 提示暂不可用 |
...
- 不接入不必要追踪 SDK。
- 不默认申请敏感权限。
- 大陆发布准备清单：`docs/release/MAINLAND_RELEASE_CHECKLIST.md`。
- 生产发布资料表：`docs/release/PRODUCTION_RELEASE_PROFILE.md`，用 `check:release-profile` 防止运营主体、域名、备案、外部服务商、法务文本和商店签名资料栏位被误删。
...
- 真实 LLM 服务商优先级。
- 图片对象存储最终供应商。
- 正式云厂商、公网域名、HTTPS 证书，并将 `deploy/nginx/yijing.conf.example` 替换为线上 Nginx 或负载均衡配置。
