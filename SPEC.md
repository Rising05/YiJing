...
| Phase 4 通义万相 + 图片存储 | In Progress | Agent-AI/Backend | 2026-07-04 | ImageService、通义万相 wan2.6-t2i HTTP 同步调用入口、图片存储抽象和本地持久化 provider 已实现；真实万相调用待 API Key 验证，OSS/S3 正式存储待账号配置 |
...
| 后端运行配置自检 | Agent-Backend/QA | Done | 新增 `check:config` 脚本，读取 `apps/server/.env` 或指定 `ENV_FILE`，在不打印密钥的前提下检查核心运行变量、真实 LLM、通义万相和对象存储配置；mock 模式下明确说明 Key 非必需，真实模式缺 Key/URL/模型/尺寸会失败，OSS 配置不完整会失败并提示当前上传删除实现仍为占位 | `apps/server/scripts/check-runtime-config.mjs`, `apps/server/package.json`, `package.json`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/check-runtime-config.mjs`; `npm run check:config`; `ENV_FILE=apps/server/.env.example npm run check:config` | 2026-07-04 |
...
| MVP 静态验收聚合命令 | Agent-QA | Done | 新增 root `check:mvp`，按顺序运行 server build、Prisma validate、配置/内容安全/AI 模板/图片存储检查、mobile build、前端密钥/权限/广告追踪 SDK 检查和发布环境报告；作为多个 agent 交接前的快速静态门禁，不替代需要运行服务的 `smoke:api` 与 `smoke:ui` | `scripts/check-mvp.mjs`, `package.json`, `README.md`, `SPEC.md` | `node --check scripts/check-mvp.mjs`; `npm run check:mvp`; `git diff --check` | 2026-07-05 |
...
| Status | Owner | 更新时间 | 证据/备注 |
| --- | --- | --- | --- |
| In Progress | Agent-AI/Backend | 2026-07-04 | 通义万相调用入口、存储抽象和本地持久化 provider 已完成；真实生图需配置 Key 验证，OSS/S3 正式存储需账号参数后接入 |
