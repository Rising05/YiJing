...
| 后端 env 示例完整性门禁 | Agent-Backend/Release/QA | Done | 新增 `check:env-examples`，检查 `apps/server/.env.example` 与 `.env.production.example` 的 42 个运行时环境变量 key 完整、顺序一致、无重复和无意外 key；同时扫描后端源码中直接引用的环境变量，防止后续新增运行配置但漏写模板。脚本排除 `API_BASE_URL`、`LIVE_AI_SMOKE`、`IMAGE_CLEANUP_DRY_RUN` 等 smoke/维护脚本临时变量，并已纳入 root `check:mvp` | `apps/server/scripts/check-env-examples.mjs`, `apps/server/package.json`, `package.json`, `scripts/check-mvp.mjs`, `README.md`, `SPEC.md` | `node --check apps/server/scripts/check-env-examples.mjs`; `npm run check:env-examples`; `npm run check:mvp`; `git diff --check` | 2026-07-06 |
